...
export function Header() {
  ...
  const isHeaderWhite = isScrolled || isHovered
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${
        isHeaderWhite 
          ? theme === "dark"
            ? "bg-slate-900/90 border-slate-800 py-0.5 shadow-md"
            : "bg-white border-border py-0.5 shadow-md" 
          : "bg-slate-950/70 backdrop-blur-xl border-white/5 py-1"
      }`}
          ...
          <Link to="/" className="flex items-center gap-2">
            <img
              ...
              className={`h-6 lg:h-7 w-auto transition-all duration-300 ${
                theme === "dark" || !isHeaderWhite ? "brightness-0 invert" : ""
              }`}
            />
          </Link>
                ...
                {item.hasDropdown && (
                  <AnimatePresence>
                    {isServicesOpen && (
                      ...
                      >
                        <div className="w-[600px] p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.15)]">
